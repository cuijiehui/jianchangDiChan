...
public class RefreshableView extends LinearLayout implements View.OnTouchListener {
    ...
    /**
     * 使用的是listview还是scrollview true是S
     */
    private boolean isScrolView=true;
    /**
     * 刷新时显示的进度条
     */
    ...
    public boolean setChildAt(View view){
    ...
    public boolean onTouch(View v, MotionEvent event) {
        if(isScrolView){
            setIsAbleToPullS(v,event);
        }else{
            setIsAbleToPull(event);
        }
        if (ableToPull) {
            LogUtils.i("下拉刷新测试event.getAction()="+event.getAction());
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    LogUtils.i("下拉刷新测试MotionEvent.ACTION_DOWN=");
                    yDown = event.getRawY();
                    break;
                case MotionEvent.ACTION_MOVE:
                    LogUtils.i("下拉刷新测试MotionEvent.ACTION_MOVE=");
                ...
                default:
                    if (currentStatus == STATUS_RELEASE_TO_REFRESH) {
                        LogUtils.i("下拉刷新测试STATUS_RELEASE_TO_REFRESH=");

                        // 松手时如果是释放立即刷新状态，就去调用正在刷新的任务
                        new RefreshingTask().execute();
                    } else if (currentStatus == STATUS_PULL_TO_REFRESH) {
                        LogUtils.i("下拉刷新测试STATUS_PULL_TO_REFRESH=");
            ...
            // 时刻记得更新下拉头中的信息
            LogUtils.i("下拉刷新测试currentStatus="+currentStatus);
